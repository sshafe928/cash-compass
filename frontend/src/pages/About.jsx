          ...
          <div className="container mx-auto flex flex-wrap justify-center gap-4 px-4">
            {visibleImages.map((src, index) => (
              <img
                ...
                src={src}
                alt={`Gallery ${index + 1}`}
                className="rounded-lg shadow-md w-full max-w-[400px] md:max-w-[300px] object-cover"
              />
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-col gap-8 justify-center mx-auto w-[90%] sm:w-[80%] mt-96 sm:mt-80 md:mt-60">
        ...
        <div className="flex gap-6 flex-col md:flex-row items-center">
          <div className="flex flex-row md:flex-col gap-4 pr-0 md:pr-4 h-full">
            <Link to="/login">
              <button className="rounded-3xl w-[180px] h-[44px] sm:w-[220px] font-semibold text-white bg-dark-blue px-4 py-2 hover:bg-hl-blue hover:text-dark-blue">
                Login
              </button>
            </Link>
            <Link to="/signup">
              <button className="rounded-3xl w-[180px] h-[44px] sm:w-[220px] border-2 border-dark-blue text-dark-blue font-semibold px-4 py-2 hover:bg-gray-300">
                Create an Account
              </button>
            </Link>
          </div>
